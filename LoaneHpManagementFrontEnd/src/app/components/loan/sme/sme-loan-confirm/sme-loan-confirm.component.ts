      ...
      <div class="mt-3">
        <ul class="list-unstyled">
          ...
          <li class="mb-2">
            ...
            <span [class.text-success]="data.paidPrincipalStatus === 'paid'"
                  [class.text-danger]="data.paidPrincipalStatus === 'not_paid'">
              <strong>{{ data.paidPrincipalStatus === 'paid' ? 'PAID' : 'NOT PAID' }}</strong>
            </span>
          </li>
        </ul>
      </div>
...
@Component({
  selector: 'app-sme-loan-confirm',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <div class="modal-header">
      <h5 class="modal-title">Confirm SME Loan</h5>
      <button type="button" class="btn-close" aria-label="Close" (click)="activeModal.dismiss()"></button>
    </div>
    <form #confirmForm="ngForm" (ngSubmit)="onSubmit(confirmForm)">
      <div class="modal-body">
        <div class="row g-3">
          ...
          <!-- Principal Status -->
          <div class="col-12">
            <label class="form-label">Principal Status</label>
            <div class="form-check">
              <input class="form-check-input" 
                     type="radio" 
                     name="paidPrincipalStatus" 
                     id="statusPaid"
                     [(ngModel)]="confirmData.paidPrincipalStatus"
                     value="paid"
                     required>
            ...
            <div class="form-check">
              <input class="form-check-input" 
                     ...
                     value="not_paid"
                     required>
              <label class="form-check-label" for="statusNotPaid">
                Not Paid
              </label>
            </div>
          </div>
...
export class SmeLoanConfirmComponent implements OnInit {
